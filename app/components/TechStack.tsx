  ...
  return (
    <section id="tech" className="editorial-container py-14 section-rule">
      ...
      <div className="mt-8 grid gap-4 2xl:grid-cols-2">
        {groups.map((group) => (
          <div key={group.title} data-reveal className="editor-card ornate-box rounded-xl p-4">
            <h3 className="text-xs font-semibold uppercase tracking-[0.18em] text-[var(--accent)]">{group.title}</h3>
            <div className="mt-4 grid grid-cols-2 gap-2">
              {group.items.map((item) => (
                <div
                  key={item}
                  className="halftone rounded-md border border-[var(--line)] px-2 py-2 text-center text-[0.66rem] uppercase tracking-[0.05em] text-[var(--ink)]"
                >
                  {item}
                </div>
              ))}
